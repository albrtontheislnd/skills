...
description: Find and rank prominent economics books for a requested year by researching curated award, bestseller, publisher, and economics-media sources, then aggregating the results into a cited Markdown report. Use for requests about the best or most notable economics books of a year, prominence-ranked reading lists, recommendation pipelines, or refreshing an existing list.
version: 1.3.0
author: Albert
license: MIT
metadata:
  hermes:
    category: reading
    tags: [Reading, Books, Economics, Research]
    related_skills: []
...
This skill has three phases: **collect**, **aggregate**, **report**. The agent does the collecting itself via `web_search` and `web_extract`; the bundled scripts perform deterministic validation, aggregation, and report rendering.

### Output location

All files generated during a run — `source_log.json`, `raw_mentions.json`, `ranked_books.json`, and the final Markdown report — must be saved in a dedicated subdirectory of the workspace root. Ask the user for the output-subfolder name before creating it; suggest `economics-book-reports` when they have not specified one, and use the confirmed name consistently for the entire run.

The workspace is **the workspace determined by the Hermes agent at runtime** (`os.getcwd()`). It is not the skill source directory (`reading/economics-book-finder/`) or the repository containing the skill. Before creating output, reject an unresolved placeholder workspace path containing both `<` and `>`. Create the confirmed output subdirectory in the workspace root if it does not already exist, and run commands from the workspace root so output paths resolve there. Preserve existing output files unless the user explicitly asks to replace them.
...
2. For each source, use `web_search` and/or `web_extract` to find its economics-book content for the target year. Award/best-of pages usually need `web_extract` directly after locating a current URL. Bestseller charts and blogs are often better reached via `web_search` first because their URLs may change or be republished each year.

For the Tier 4 "open domain search" list in `{HERMES_SKILL_DIR}/references/sources.md`, there is no single page to fetch. Run each domain through `web_search` using the query templates given there (for example, `site:domain.com "economics book" [year]`), then use `web_extract` on relevant results. Treat each relevant article as one mention at Tier 4's weight; do not relabel it as Tier 3.

4. **Immediately after checking each source — not in a batch at the end — append one entry to `<workspace>/<output-subfolder>/source_log.json`** recording what happened, using status `"ok"` (found mentions), `"empty"` (checked, nothing relevant), or `"failed"` (couldn't access — must include a `notes` reason). Include `mentions_found` for every entry. See the schema and rationale in `{HERMES_SKILL_DIR}/scripts/check_coverage.py`'s docstring.
...
6. **Once you believe the sweep is complete, run the coverage gate — do not proceed to Phase 2 until it exits 0:**
   ...
   ```bash
   python {HERMES_SKILL_DIR}/scripts/check_coverage.py --sources {HERMES_SKILL_DIR}/references/sources.md --log <output-subfolder>/source_log.json
   ```
...
Save this as `<workspace>/<output-subfolder>/raw_mentions.json`.
...
```bash
python {HERMES_SKILL_DIR}/scripts/aggregate_books.py <output-subfolder>/raw_mentions.json --output <output-subfolder>/ranked_books.json
```
...
- Confirm the requested year and the confirmed output-subfolder name.
